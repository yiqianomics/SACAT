...
check_packages <- function(packages) {
    ...
    if (length(missing)) {
        stopf(
            "Missing required packages: %s. Install them and run preflight again.",
...
summarize_completed_replications <- function() {
    ...
    raw_results_path <- file.path(
        CONFIG$summary_dir, "combined_taxon_method_results.csv.gz"
    )
    raw_truth_path <- file.path(
        CONFIG$summary_dir, "combined_simulation_truth.csv.gz"
    )
    metrics_parts <- list()
    failure_parts <- list()
    truth_parts <- list()
    ...
    data.table::fwrite(
        metrics,
        file.path(CONFIG$summary_dir, "combined_replication_metrics.csv")
    )
    data.table::fwrite(
        metric_summary,
        file.path(CONFIG$summary_dir, "combined_setting_summary.csv")
    )
    data.table::fwrite(
        failure_summary,
