...
# The archive contains an RDP-assigned 100% de novo feature table generated by
# the MicrobiomeHD pipeline. This is not the 97% OTU table used in the original
# publication.
#
# The publication retained samples with at least 1,450 sequences and then
# rarefied them to 1,450 sequences. For the present analysis, the standard
# MicrobiomeHD count filters are first applied to all samples shared by the
# count table and metadata. The publication's sample-depth threshold is then
# used, but counts are not rarefied because DASRA models the original library
# size. A 5% feature-prevalence rule is applied to the CDI-versus-healthy
# cohort as a prespecified analysis filter; it was not part of the publication's
# sequence-curation protocol. Depth filtering alone retains 153 healthy and 91
